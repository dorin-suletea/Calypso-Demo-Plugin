...
import implementation.forms.InheritanceConnectorContextUIPan;
...
public class InheritanceContext implements ConnectorContextInterface {
	...
	public void drawEndArrow(Graphics g, ArrayList<Point> points) {
		...
		g.fillPolygon(new int[]{points.get(1).x,points.get(3).x,points.get(5).x},
					  new int[]{points.get(1).y,points.get(3).y,points.get(5).y}, 
					  3);
		g.setColor(gCol);
		...
		g.drawLine(points.get(3).x, points.get(3).y, points.get(5).x, points.get(5).y);
	}
	...
	public AbstractControlJPanel generateUiPan(ConnectorViewProxy connector) {
		return new InheritanceConnectorContextUIPan(connector);
	}
