...
import implementation.forms.AbstractClassContextUIPan;
...
public class AbstractClassContext implements EntityContextInterface {
	private final int TEXT_H_PAD = 5;
	private final int DATASHETT_H_PAD = 30;
	...
	public void drawShape(Graphics g, int width, int height) {
		g.drawArc(5, 5 , width-10, 100, 0, 180);
		g.drawLine(5, 50, 5, height-10);
		g.drawLine(width-5, 50, width-5, height-10);
		g.drawLine(5, height-10, width-10, height-10);
	}
	...
	public void setNestedPositions(List<GlyphLocationInterface> nested, GlyphLocationInterface hostView) {
		...
		// n.setBounds(n.getBounds().x,50,100,100);
	}

	@Override
	public AbstractControlJPanel generateControlPannel(EntityViewProxy entity) {
		return new AbstractClassContextUIPan(entity);
	}
	...
	public Dimension getMinimumSize(EntityModelInterface entityModel, FontMetrics fontMetrics) {
		...
		width+=20;//because it's a circle and string is going to be out of bounds
